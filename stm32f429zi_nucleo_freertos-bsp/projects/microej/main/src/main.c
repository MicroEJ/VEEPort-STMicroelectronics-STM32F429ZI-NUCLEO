...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  /* Display MCU ref */
  printf( "MCU ID %x rev %x \r\n", HAL_GetDEVID(), HAL_GetREVID());

  /* Display frequency */
  printf( "Sys: %i / HCLK: %i / PCLK1: %i \r\n", HAL_RCC_GetSysClockFreq (),HAL_RCC_GetHCLKFreq(), HAL_RCC_GetPCLK1Freq());

  printf("Hardware init started...\n");

#ifdef VALIDATION
  T_CORE_main();
#endif
  TaskHandle_t pvCreatedTask;	
  xTaskCreate( xMicroEJVEETaskFunction, "MicroEJ VEE", MICROEJ_VEE_TASK_STACK_SIZE, NULL, MICROEJ_VEE_TASK_PRIORITY, &pvCreatedTask );
