...
"%IAREW_INSTALLATION_DIR%\common\bin\cspybat" --download_only "%IAREW_INSTALLATION_DIR%\arm\bin\armproc.dll" "%IAREW_INSTALLATION_DIR%\arm\bin\armstlink2.dll" "%IAREW_PROJECT_DIR%\%IAREW_PROJECT_CONFIGURATION%\Exe\%IAREW_PROJECT_NAME%.out" --plugin="%IAREW_INSTALLATION_DIR%\arm\bin\armbat.dll" --device_macro="%IAREW_INSTALLATION_DIR%\%IAREW_PROJECT_DEVICE_MACRO%" --flash_loader="%IAREW_INSTALLATION_DIR%\%IAREW_PROJECT_FLASH_LOADER%" --backend "--endian=little" "--cpu=Cortex-M4" "--fpu=VFPv4_SP" "-p" "%IAREW_INSTALLATION_DIR%\arm\config\debugger\ST\STM32F429ZI.ddf" "--semihosting" "--device=STM32F429ZI" "--drv_interface=SWD" "--stlink_reset_strategy=0,0" "--drv_swo_clock_setup=168000000,0,2000000" "--drv_communication_log=%~dp0\cspycomm.log" "--drv_catch_exceptions=0x000" "--drv_debug_ap=0" "--stlink_probe=stlinkv2"
